...
lazy_static! {
    ...
    pub static ref ACL_HEADER: HeaderName = HeaderName::from_static("x-amz-acl");
    /// Date header used in authorization.
    pub static ref DATE_HEADER: HeaderName = HeaderName::from_static("x-amz-date");
    /// Hash header used in authorization.
    pub static ref HASH_HEADER: HeaderName = HeaderName::from_static("x-amz-content-sha256");
...
pub(crate) trait RequestBuilder {
    ...
    fn query_string(&self) -> String {
        ...
        for (name, value) in self.raw_params() {
            ...
            string.push_str(
                &url_encode::utf8_percent_encode(name, DEFAULT_ENCODE_SET).collect::<String>(),
            );
            string.push('=');
            string.push_str(
                &url_encode::utf8_percent_encode(value, DEFAULT_ENCODE_SET).collect::<String>(),
            );
        }
...
// NOTE: This struct is accessible only within the crate, so we don't have
// to worry about the fields and methods being public.
